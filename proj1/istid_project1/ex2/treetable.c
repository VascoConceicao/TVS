...
int balanced(TreeTable *t)
{
    ...
    struct frame {
        RBNode *node;
        // state where we are in traversal,
        // 0 = we just arrived at this node, process left node
        // 1 = finished processing left, go process right
        // 2 = done, compare heights
        int state;
        int left_height;
        int right_height;
    };

    struct frame *stack = t->mem_alloc(t->size * sizeof(struct frame));
    if (!stack)
        return 0;

    size_t top = 0; // size/position of the stack
    int ok = 1;

    stack[top++] = (struct frame) { t->root, 0, 0, 0 };

    while (top > 0 && ok) {
        struct frame *current = &stack[top - 1]; // indexed by 0

        if (current->state == 0) { // check left
            current->state = 1;
            if (current->node->left != t->sentinel) // until null
                stack[top++] = (struct frame) { current->node->left, 0, 0, 0 };
        } else if (current->state == 1) { // check right
            current->state = 2;
            if (current->node->right != t->sentinel) // until null
                stack[top++] = (struct frame) { current->node->right, 0, 0, 0 };
        } else {
            // if we are here, both heights are already known
            int diff = current->left_height - current->right_height;
            if (diff < -1 || diff > 1) {
                ok = 0;
                break;
            }
            ...
            height = (current->left_height > current->right_height ?
                      current->left_height : current->right_height) + 1;
            top--; // pass height to parent

            if (top > 0) {
                if (stack[top - 1].state == 1) // update parent left height
                    stack[top - 1].left_height = height;
                else                           // update parent right height
                    stack[top - 1].right_height = height;
            }
        }
    }
...
int sorted(TreeTable *t)
{
    ...
    RBNode **stack = t->mem_alloc(t->size * sizeof(RBNode *));
    if (!stack)
        return 0;
    ...
    while ((node != t->sentinel || top > 0) && ok) {
        while (node != t->sentinel) { // traverse left
            stack[top++] = node;
            node = node->left;
        }

        node = stack[--top]; // pop left most node

        if (previous && t->cmp(previous->key, node->key) >= 0) { // left > node > right 
            ok = 0;
            break;
        }
